#!/usr/bin/env Rscript
# ------------------------------------------------------------
# scripts/dge/04_run_edger_sex_effects.R
#
# Purpose
#   Run edgeR QL differential expression for SEX-stratified effects
#   using grouped factor: sex_treat_time = sex_Treatment_Time
#
...
#
# Output
#   - results/dge/model_sex_treat_time/tables/<contrast>.all.tsv
#   - results/dge/model_sex_treat_time/tables_filtered/<contrast>.fdr25_logfc1p25.tsv
#   - results/dge/model_sex_treat_time/summary/sexDiffGenes.tsv
#   - results/dge/model_sex_treat_time/gene_lists/*.txt
#
# Run (from repo root)
#   Rscript scripts/dge/04_run_edger_sex_effects.R
# ------------------------------------------------------------
...
model_dir   <- file.path(repo_root, "results", "dge", "model_sex_treat_time")
...
if (!"sex" %in% colnames(meta_df)) stop("metadata.tsv must contain sex column.")
...
meta_df$Sex2      <- normalize_sex(meta_df$sex)
...
if (anyNA(meta_df$Sex2)) {
  bad <- unique(meta_df$sex[is.na(meta_df$Sex2)])
  stop("Unmapped sex values in metadata:\n  ", paste(bad, collapse = "\n  "))
}

meta_df$sex_treat_time <- factor(paste(meta_df$Sex2, meta_df$Treatment, meta_df$Time_hr, sep = "_"))
...
keep <- filterByExpr(y, group = meta_df$sex_treat_time)
y <- y[keep, , keep.lib.sizes = FALSE]
y <- calcNormFactors(y, method = "TMM")

design <- model.matrix(~0 + sex_treat_time, data = meta_df)
colnames(design) <- levels(meta_df$sex_treat_time)

y <- estimateDisp(y, design)
fit <- glmQLFit(y, design)

needed_levels <- c(
  "male_sham_0","male_sham_3","male_sham_24",
  "male_stroke_0","male_stroke_3","male_stroke_24",
  "male_treated_0","male_treated_3","male_treated_24",
  "female_sham_0","female_sham_3","female_sham_24",
  "female_stroke_0","female_stroke_3","female_stroke_24",
  "female_treated_0","female_treated_3","female_treated_24"
)

missing_lvls <- setdiff(needed_levels, colnames(design))
if (length(missing_lvls) > 0) {
  stop(
    "Design is missing required sex_treat_time levels:\n  ",
    paste(missing_lvls, collapse = "\n  "),
    "\nCheck that metadata includes both sexes across sham/stroke/treated at 0,3,24."
  )
}

contrast_mat <- makeContrasts(
  stroke_at_reperfusion_sex_effect =
    (male_stroke_3  - male_stroke_0)  - (female_stroke_3 - female_stroke_0),

  stroke_at_followup_sex_effect =
    (male_stroke_24 - male_stroke_3)  - (female_stroke_24 - female_stroke_3),

  treatment_at_reperfusion_sex_effect =
    (male_treated_3 - male_treated_0) - (female_treated_3 - female_treated_0),

  treatment_at_followup_sex_effect =
    (male_treated_24 - male_treated_3) - (female_treated_24 - female_treated_3),
...
sexDiffGenes <- data.frame(Geneid = sort(unique(unlist(lapply(all_tables, function(d) d$Geneid)))), stringsAsFactors = FALSE)
for (nm in names(all_tables)) {
  tmp <- all_tables[[nm]][, c("Geneid","logFC")]
  colnames(tmp) <- c("Geneid", nm)
  sexDiffGenes <- merge(sexDiffGenes, tmp, by = "Geneid", all.x = TRUE, sort = FALSE)
}
sexDiffGenes <- sexDiffGenes[rowSums(is.na(sexDiffGenes[, setdiff(colnames(sexDiffGenes), "Geneid"), drop = FALSE])) < (ncol(sexDiffGenes) - 1), , drop = FALSE]

fwrite(sexDiffGenes, file.path(sum_dir, "sexDiffGenes.tsv"), sep = "\t", quote = FALSE, na = "NA")
...
if (all(c("stroke_at_reperfusion_sex_effect","treatment_at_reperfusion_sex_effect") %in% colnames(sexDiffGenes))) {
  restoredFunction <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_reperfusion_sex_effect > 0 &
                                           sexDiffGenes$stroke_at_reperfusion_sex_effect < 0]
  upregulated_treated_3h <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_reperfusion_sex_effect > 0 &
                                                 is.na(sexDiffGenes$stroke_at_reperfusion_sex_effect)]
  change_in_regulation_treated_3h <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_reperfusion_sex_effect < 0 &
                                                          sexDiffGenes$stroke_at_reperfusion_sex_effect > 0]
  downregulated_treated_3h <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_reperfusion_sex_effect < 0 &
                                                   is.na(sexDiffGenes$stroke_at_reperfusion_sex_effect)]
  furtherdown_treated_3h <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_reperfusion_sex_effect <
                                                 sexDiffGenes$stroke_at_reperfusion_sex_effect &
                                                 sexDiffGenes$stroke_at_reperfusion_sex_effect < 0]
...
if (all(c("stroke_at_followup_sex_effect","treatment_at_followup_sex_effect") %in% colnames(sexDiffGenes))) {
  restoredFunction_followup <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_followup_sex_effect > 0 &
                                                    sexDiffGenes$stroke_at_followup_sex_effect < 0]
  upregulated_treated_followup <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_followup_sex_effect > 0 &
                                                        is.na(sexDiffGenes$stroke_at_followup_sex_effect)]
  change_in_regulation_treated_followup <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_followup_sex_effect < 0 &
                                                                 sexDiffGenes$stroke_at_followup_sex_effect > 0]
  downregulated_treated_followup <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_followup_sex_effect < 0 &
                                                          is.na(sexDiffGenes$stroke_at_followup_sex_effect)]
  furtherdown_treated_followup <- sexDiffGenes$Geneid[sexDiffGenes$treatment_at_followup_sex_effect <
                                                        sexDiffGenes$stroke_at_followup_sex_effect &
                                                        sexDiffGenes$stroke_at_followup_sex_effect < 0]
