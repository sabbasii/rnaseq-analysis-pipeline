#!/usr/bin/env Rscript
# ------------------------------------------------------------
# scripts/dge/plots/volcano_sex_time.R
#
# Purpose
#   Volcano plots for sex-by-time contrasts from:
#     results/dge/model_sex_time/tables/*.all.tsv
#
...
#
# Inputs
#   - results/dge/model_sex_time/tables/sex_at_reperfusion.all.tsv
#   - results/dge/model_sex_time/tables/sex_at_followup.all.tsv
#
# Outputs
#   - results/dge/model_sex_time/plots/volcano_sex_3h.png
#   - results/dge/model_sex_time/plots/volcano_sex_24h.png
#
# Run (from repo root)
#   Rscript scripts/dge/plots/volcano_sex_time.R
# ------------------------------------------------------------
...
tbl_dir <- file.path(repo_root, "results", "dge", "model_sex_time", "tables")
plots_dir <- file.path(repo_root, "results", "dge", "model_sex_time", "plots")
dir.create(plots_dir, recursive = TRUE, showWarnings = FALSE)

rep_path <- file.path(tbl_dir, "sex_at_reperfusion.all.tsv")
fol_path <- file.path(tbl_dir, "sex_at_followup.all.tsv")

need_file(rep_path, "sex_at_reperfusion.all.tsv")
need_file(fol_path, "sex_at_followup.all.tsv")

rep_df <- fread(rep_path, data.table = FALSE)
fol_df <- fread(fol_path, data.table = FALSE)

make_volcano(rep_df, file.path(plots_dir, "volcano_sex_3h.png"))
make_volcano(fol_df, file.path(plots_dir, "volcano_sex_24h.png"))
